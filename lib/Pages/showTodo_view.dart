...
class ShowTodosView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocListener(
      listeners: [
        BlocListener<TodoListsCubit,TodoListState>(listener: (context,state){
          context.read<FilteredTodoCubisCubit>().setFilteredTodos(context.read<TodoFilterCubit>().state.filter,
              state.todos,context.read<TodoSearchCubit>().state.searchText);
        }),
        BlocListener<TodoFilterCubit,TodoFilterCubitState>(listener: (context,state){
          context.read<FilteredTodoCubisCubit>().setFilteredTodos(
              state.filter,
              context.read<TodoListsCubit>().state.todos,context.read<TodoSearchCubit>().state.searchText);
        }),
        BlocListener<TodoSearchCubit,TodoSearchCubitState>(listener: (context,state){
          context.read<FilteredTodoCubisCubit>().setFilteredTodos(
              context.read<TodoFilterCubit>().state.filter,
              context.read<TodoListsCubit>().state.todos,state.searchText);
        })
      ],
